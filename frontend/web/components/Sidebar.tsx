      ...
      <div className="p-6 pb-2">
        <div className="flex items-center p-3 bg-gray-50 rounded-xl border border-gray-100">
          <div className="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center text-[#EE501C] font-bold text-sm">
            {localStorage.getItem('adminEmail')?.charAt(0).toUpperCase() || 'AD'}
          </div>
          <div className="ml-3 overflow-hidden">
            <p className="text-sm font-semibold text-gray-900 truncate">{localStorage.getItem('adminName') || 'Admin'}</p>
            <p className="text-xs text-gray-500 truncate">{localStorage.getItem('adminEmail') || 'admin@food.com'}</p>
          </div>
        </div>
      </div>
