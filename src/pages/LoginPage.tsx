...
export const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, onRegister, onForgotPassword, onBack, onHome }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 1. Kiểm tra tài khoản admin cứng (Vẫn giữ để debug nhanh nếu cần)
    ...
    // 2. Kiểm tra trong danh sách người dùng thực tế
    const users = userService.getUsers();
    const user = users.find(u => u.email === email);

    // Kiểm tra mật khẩu đã lưu (nếu chưa có thì mặc định là 123456 cho các user cũ)
    ...
    <div className="min-h-screen relative flex flex-col items-center bg-white overflow-hidden font-sans px-4 pt-28 pb-10">
      ...
      <div className="bg-white/70 backdrop-blur-md rounded-[40px] p-10 sm:p-12 w-full max-w-[550px] border border-gray-100 shadow-sm relative z-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button 
            type="submit" 
            className="w-full py-4 bg-primary-600 text-white font-bold rounded-[30px] text-lg hover:bg-primary-700 hover:-translate-y-0.5 transition-all duration-300"
          >
            ĐĂNG NHẬP
          </button>
        </form>
        ...
        <div className="relative text-center my-8">
          ...
          <span className="relative z-10 bg-white/50 px-4 text-sm font-medium text-gray-500 rounded-full">Hoặc đăng nhập với</span>
        </div>
        ...
        <div className="flex justify-center gap-5 mb-8">
          {[
            ...
            <button key={i} className="w-14 h-14 rounded-full bg-white border border-gray-200 flex items-center justify-center text-gray-700 hover:border-primary-500 hover:-translate-y-1 transition-all duration-300">
