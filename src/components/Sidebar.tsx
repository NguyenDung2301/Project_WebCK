    ...
    <div className="w-64 bg-white border-r border-gray-200 h-screen fixed left-0 top-0 flex flex-col z-10 hidden md:flex shadow-sm">
      ...
      <div className="p-6 pb-2">
        <div className="flex items-center p-3 bg-gray-50 rounded-xl border border-gray-100">
          ...
          <div className="ml-3 overflow-hidden">
            ...
            <p className="text-xs text-gray-500 truncate">admin@food.com</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
        {menuItems.map((item) => (
          ...
          >
            <span className={`${activeTab === item.id ? 'text-primary-600' : 'text-gray-400 group-hover:text-gray-500'} mr-3`}>
              {item.icon}
            </span>
            {item.label}
          </button>
